...
void raise_intr(uint8_t NO){
 ...
 cpu.esp-=4;
  ...
  cpu.esp-=2;
  swaddr_write(cpu.esp,2,cpu.segReg[S_CS].visiblePart,S_SS);
  cpu.esp-=4;
  swaddr_write(cpu.esp,4,cpu.eip,S_SS);

  uint32_t low= lnaddr_read(cpu.idtr.baseAddr+sizeof(struct GateDescriptor)*NO,4);
  uint32_t high= lnaddr_read(cpu.idtr.baseAddr+sizeof (struct GateDescriptor)*NO +4,4);
